// src/pages/Phase3.tsx - Complete Working Phase 3 Page
...
const Phase3 = () => {
    ...
    }, [wallet.publicKey, wallet.connecting, program, games]);
    ...
    useEffect(() => {
        ...
        if (game) {
            ...
            console.log('🎮 Phase3 Started:', game.phase3Started);
            setCurrentGame(game);
    ...
    // ✅ Check if phase3 already started and redirect
    useEffect(() => {
        if (!currentGame || hasCheckedPhase3Status.current || showBattleGame) return;

        if (currentGame.phase3Started && readyStates.length > 0) {
            const readyCount = readyStates.filter(s => s.ready).length;

            if (readyCount >= 2) {
                hasCheckedPhase3Status.current = true;
                hasTriedAutoAdvance.current = true;

                console.log('🎮 Phase 3 already started on load, redirecting to battle...');
    ...
    const updateTimer = () => {
        ...
        // ✅ If phase3 already started with 2+ players, stop timer
        if (currentGame.phase3Started && readyStates.filter(s => s.ready).length >= 2) {
            setTimeRemaining('BATTLE ACTIVE');
            setCanAdvance(false);
            return;
        }
        ...
        if (diff <= 0) {
            ...
            if (!hasTriedAutoAdvance.current && !currentGame.phase3Started) {
                hasTriedAutoAdvance.current = true;
        ...
        <div className="min-h-screen bg-background relative overflow-hidden">
            ...
            <main className="relative z-10 container mx-auto px-4 pt-24 pb-16">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                    ...
                    <Card className="mb-4 border-yellow-600 bg-yellow-900/20">
                        <CardContent className="pt-4">
                            <div className="text-xs space-y-1">
                                <p><strong>DEBUG:</strong></p>
                                <p>Phase3 Started: {currentGame?.phase3Started ? '✅ YES' : '❌ NO'}</p>
                                <p>Ready Count: {readyCount}/{totalPlayers}</p>
                                <p>Show Battle Game: {showBattleGame ? '✅ YES' : '❌ NO'}</p>
                                <p>Has Checked: {hasCheckedPhase3Status.current ? '✅' : '❌'}</p>
                                <p>Has Auto Advanced: {hasTriedAutoAdvance.current ? '✅' : '❌'}</p>
                                <Button
                                    onClick={() => {
                                        console.log('🔧 Manual trigger showBattleGame');
                                        setShowBattleGame(true);
                                    }}
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-bold">
                                        {canAdvance ? 'TIME EXPIRED!' : timeRemaining === 'BATTLE ACTIVE' ? 'BATTLE IN PROGRESS' : 'Time Remaining'}
                                    </p>
                                </div>
                            </div>
                            <div className="text-center">
                                <p className={`text-5xl font-mono font-black ${canAdvance ? 'text-red-500 animate-pulse' :
                                    timeRemaining === 'BATTLE ACTIVE' ? 'text-green-500 animate-pulse' :
                                        'text-red-400'
                                    }`}>
                                    {timeRemaining}
                                </p>
